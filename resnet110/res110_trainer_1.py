...
def main():
    ...
    model_para = torch.load('../res110/res110-ori.th')
    ...
    layer_prune = [6, 8, 12, 14, 18, 20, 24, 26, 30, 32, 36, 38, 42, 44, 48, 50, 54, 56, 60, 62, 66, 68,
                   72, 74, 78, 80, 84, 86, 90, 92, 96, 98, 102, 104, 108, 110,
                   115, 117, 121, 123, 127, 129, 133, 135, 139, 141, 145, 147, 151, 153, 157, 159, 163, 165,
                   169, 171, 175, 177, 181, 183, 187, 189, 193, 195, 199, 201, 205, 207, 211, 213, 217, 219,
                   224, 226, 230, 232, 236, 238, 242, 244, 248, 250, 254, 256, 260, 262, 266, 268, 272, 274,
                   278, 280, 284, 286, 290, 292, 296, 298, 302, 304, 308, 310, 314, 316, 320, 322, 326, 328]
    indices_pruned = {}
    for i in range(num_prune):
        # load pruned channel
        indices_pruned['pruned_filter' + str(2 * i)] = load_pruning_file(int(2 * i + 1))
        indices_pruned['pruned_filter' + str(2 * i + 1)] = load_pruning_file(int(2 * i + 2))
        # pruning
        pruning(model, layer_prune[int(2 * i)], indices_pruned['pruned_filter' + str(2 * i)])
        pruning(model, layer_prune[int(2 * i + 1)], indices_pruned['pruned_filter' + str(2 * i + 1)])
...
#############################################################################################
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                        milestones=[30, 80], last_epoch=args.start_epoch - 1)
    ...
    ############################################################################################################################
    file_name = open('./res110_acc-' + str(num_prune) + '.txt', mode='w')
...
def load_pruning_file(layer_num):
    ...
    # f = open('./4.Pruned_Label/pruned_label_res110_0.50/FM' + str(layer_num) + '.txt', 'r')
