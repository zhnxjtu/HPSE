...
def main():
    ...
    pretrained_dict = torch.load('./resnet50-19c8e357.pth')  # change
    ...
    train_dataset = torchvision.datasets.ImageFolder(root='/data/ILSVRC2012_pytorch/train',
    ...
    val_dataset = torchvision.datasets.ImageFolder(root='/data/ILSVRC2012_pytorch/val',
                                                    transform=transforms.Compose([
    ...
    prec1_ori, prec5_ori = validate(val_loader, model, criterion)
    print('Test Top-1 accuracy: \033[31m {:.2f} \033[0m, Top-5 accuracy: {:.2f}'.format(prec1_ori, prec5_ori))
    ...
    prec1_1time, prec5_1time = validate(val_loader, model, criterion)
    print('Test Top-1 accuracy: \033[31m {:.2f} \033[0m, Top-5 accuracy: {:.2f}'.format(prec1_1time, prec5_1time))
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[25, 45], gamma=0.1)
...
def load_pruning_file(layer_num):
    ...
    # f = open('./4.Pruned_Label/pruned_label_res50_0.43/FM' + str(layer_num) + '.txt', 'r')
